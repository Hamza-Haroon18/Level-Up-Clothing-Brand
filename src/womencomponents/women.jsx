...
import { useSearchParams } from 'react-router-dom'; // ← URL query parameters ka hook
import Header from './header';
import Footer from './footer';
import Add_data from './add_data';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
...
function Women() {
  const [currentList, setCurrentList] = useState('');
  const [searchParams, setSearchParams] = useSearchParams(); // ← Hook for query params

  // Convert query params into JS object
  ...
  // Refresh par bhi selectedFilters ko update karo (sync with URL)
  useEffect(() => {
  ...
  const handleSelect = (group, value) => {
    ...
    // Remove null (matlab deselect hua to URL se hata do)
    Object.keys(updated).forEach(key => {
      if (!updated[key]) delete updated[key];
    });

    setSelectedFilters(updated);
    setSearchParams(updated); // ← Set to URL query params
  };
  ...
  const options = {
    ...
    styling: ['Casual', 'Formal', 'Luxury', 'Printed', 'Embroidered'],
    wear: ['Casual', 'Partywear', 'Festive', 'Office Wear'],
    trouser: ['Dyed Trouser', 'Printed Trouser', 'Embroidered Trouser', 'Plain Trouser','No Trouser'],
    color: ['Pastels', 'Brights', 'Neutrals', 'Dark Shades', 'Multicolor'],
