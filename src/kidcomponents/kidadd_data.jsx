import './kidadd_data.css'
import { Link } from 'react-router-dom'
import { useState, useRef, useEffect } from 'react'
import ProductPage from './kidshow-product';
function Add_data({ filters }) {
    ...
    useEffect(() => {
        fetch(`http://localhost:3000/admin/kid?page=${page}&limit=12`)
        ...
        <>
            <div className='sort-headers'>
                ...
                <div className='item2'>
                    ...
                    <button><Link to="/admin/kid/kidadd-product">Add product</Link></button>
                </div>
            ...
            <div className="innercollection">
                {filteredProducts.map((key, index) => (
                    <div className="box" key={index}>
                        <Link to={`/admin/kidshow-product/${key._id}`}><div className="image_box"><img src={`http://localhost:3000/uploads/${key.image}`} alt={key.productname} />
                        </div>
                        ...
                        <button  className="edit-btn"><Link to={`/admin/kid/kidedit-product/${key._id}`}>Edit Product</Link></button>
                        <button className="delete-btn"><Link to={`/admin/kiddelete-product/${key._id}`}>Delete Product</Link></button>
                    </div>
                ))}
            </div>
