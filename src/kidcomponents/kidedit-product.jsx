...
function Editproductkid({ }) {
    ...
    useEffect(() => {
        fetch(`http://localhost:3000/admin/kid/kidedit-product/${id}`)
            .then((res) => res.json())
            .then((data) => {
    ...
    const redirectsubmit = async (e) => {
        ...
        // formData.append('image', file)
        try {
            const res = await fetch(`http://localhost:3000/admin/kid/kidedit-product/${id}`, {
                method: 'POST',
                body: formData
            })
            if (res.ok) {
