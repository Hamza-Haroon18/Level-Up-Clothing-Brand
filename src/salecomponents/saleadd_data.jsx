import './saleadd_data.css'
import { Link } from 'react-router-dom'
import { useState, useRef, useEffect } from 'react'
import ProductPage from './saleshow-product';
function Add_data({ filters }) {
    ...
    useEffect(() => {
        fetch(`http://localhost:3000/admin/salewomen?page=${page}&limit=12`)
        ...
        <>
            <div className='sort-headers'>
                ...
                <div className='item2'>
                    ...
                    <button><Link to="/admin/salewomen/saleadd-product">Add product</Link></button>
                </div>
            ...
            <div className="innercollection">
                {filteredProducts.map((key, index) => (
                    ...
                    //     <Link to={`/admin/saleshow-product/${key._id}`}><div className="image_box"><img src={`http://localhost:3000/uploads/${key.image}`} alt={key.productname} />
                    //     </div>

                    //         <h4 className="image_heading">{key.productname}</h4></Link>
                    //     <div className="price-box">
                    //         {key.discount > 0 && (
                    //             <span className="discount-badge">-{key.discount}%</span>
                    //         )}
                    //         <div className="prices">
                    //             <span className="new-price">Rs {key.finalprice}</span>
                    //             <span className="old-price">Rs {key.price}</span>
                    //         </div>
                    //     </div>


                    //     {/* <button><Link to={`/show-product/${key._id}`}>Show Product</Link></button> */}
                    //     <button className="edit-btn"><Link to={`/admin/salewomen/saleedit-product/${key._id}`}>Edit Product</Link></button>
                    //     <button className="delete-btn"><Link to={`/admin/saledelete-product/${key._id}`}>Delete Product</Link></button>
                    // </div>
                    <div className="box" key={index}>
                        <div className="image_box">
                            ...
                            <Link to={`/admin/saleshow-product/${key._id}`}>
                                <img src={`http://localhost:3000/uploads/${key.image}`} alt={key.productname} />
                            </Link>
                        </div>
                        ...
                        <button className="edit-btn">
                            <Link to={`/admin/salewomen/saleedit-product/${key._id}`}>Edit Product</Link>
                        </button>
                        <button className="delete-btn">
                            <Link to={`/admin/saledelete-product/${key._id}`}>Delete Product</Link>
                        </button>
                    </div>
