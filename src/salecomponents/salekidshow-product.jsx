...
import './salekidshow-product.css';
import { useEffect, useState } from 'react';
import { Link, useParams } from 'react-router-dom';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faCaretDown } from '@fortawesome/free-solid-svg-icons';

function ProductPagekidsale() {
  const [items, setitems] = useState(null);
  const [information, setInformation] = useState(false);
  const { id } = useParams();

  useEffect(() => {
    fetch(`http://localhost:3000/admin/salekidshow-product/${id}`)
      .then(res => res.json())
      .then(data => {
        ...
        {items ? (
          <>
            <div className="product-image">
              ...
              <button><Link to={`/admin/salekid/salekidedit-product/${id}`}>Edit Product</Link></button>
              <button><Link to={`/admin/salekiddelete-product/${id}`}>Delete Product</Link></button>
            </div>
            ...
            <div className="product-info">
              ...
              <h1 className="detail-toggle" onClick={() => setInformation(!information)}>
                More Details
...
export default ProductPagekidsale;
