import './salekidadd-product.css'
import { useState, useRef, useEffect } from 'react'

import { useParams } from 'react-router-dom';
function Editproductkidsale({ }) {
    const { id } = useParams();
    ...
    useEffect(() => {
        fetch(`http://localhost:3000/admin/salekid/salekidedit-product/${id}`)
            .then((res) => res.json())
            .then((data) => {
    ...
    const redirectsubmit = async (e) => {
        ...
        // formData.append('image', file)
        try {
            const res = await fetch(`http://localhost:3000/admin/salekid/salekidedit-product/${id}`, {
                method: 'POST',
                body: formData
            })
            if (res.ok) {
    ...
    return (
        <div className="modal-overlay">
            <form className="modal-form" onSubmit={redirectsubmit}>
                ...
                <label>Gender</label>
                <select value={gender}  onChange={(e) => setproductgender(e.target.value)} required>
...
export default Editproductkidsale;
