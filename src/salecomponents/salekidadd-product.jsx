...
function Addproductkidsale({ onClose }) {
    const [maincategory, setmaincategory] = useState('salekid')
    ...
    const redirectsubmit = async (e) => {
        ...
        try {
            const res = await fetch('http://localhost:3000/admin/salekid/salekidadd-product', {
                method: 'POST',
                body: formData
            })
            if (res.ok) {
...
export default Addproductkidsale;
