...
function Editproductsale({ }) {
    ...
    useEffect(() => {
        fetch(`http://localhost:3000/admin/salewomen/saleedit-product/${id}`)
            .then((res) => res.json())
            .then((data) => {
  ...
  formData.append('image', file); 
        ...
        // formData.append('image', file)
        try {
            const res = await fetch(`http://localhost:3000/admin/salewomen/saleedit-product/${id}`, {
                method: 'POST',
                body: formData
            })
            if (res.ok) {
