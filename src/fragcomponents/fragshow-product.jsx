...
import './fragshow-product.css';
import { useEffect, useState } from 'react';
import { Link, useParams } from 'react-router-dom';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faCaretDown } from '@fortawesome/free-solid-svg-icons';

function ProductPagefrag() {
  const [items, setitems] = useState(null);
  const [information, setInformation] = useState(false);
  const { id } = useParams();

  useEffect(() => {
    fetch(`http://localhost:3000/admin/fragshow-product/${id}`)
      .then(res => res.json())
      .then(data => {
        ...
        {items ? (
          <>
            <div className="product-images">
              ...
              <button><Link to={`/admin/frag/fragedit-product/${id}`}>Edit Product</Link></button>
              <button><Link to={`/admin/fragdelete-product/${id}`}>Delete Product</Link></button>
            </div>
...
export default ProductPagefrag;
