...
function Addproductfrag({ onClose }) {
    const [maincategory, setmaincategory] = useState('frag')
    ...
    const redirectsubmit = async (e) => {
        e.preventDefault();
        if (
            !productname || !price || !quantity || !category || !fragranceType ||
        ...
        try {
            const res = await fetch('http://localhost:3000/admin/frag/fragadd-product', {
                method: 'POST',
                body: formData
            });
            if (res.ok) {
