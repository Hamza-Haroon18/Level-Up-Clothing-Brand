import './fragadd-product.css';
import { useState, useEffect } from 'react';
import { useParams } from 'react-router-dom';

function Editproductfrag() {
    const { id } = useParams();
    ...
    useEffect(() => {
        fetch(`http://localhost:3000/admin/frag/fragedit-product/${id}`)
            .then((res) => res.json())
            .then((data) => {
    ...
    const redirectChange = () => {
        window.location.href = '/admin/frag';
    };

    const redirectsubmit = async (e) => {
        e.preventDefault();
        if (
            !productname || !price || !quantity || !category || !fragranceType || !gender ||
        ...
        try {
            const res = await fetch(`http://localhost:3000/admin/frag/fragedit-product/${id}`, {
                method: 'POST',
                body: formData
            });
            if (res.ok) {
