...
import './skincaresshow-product.css';
import { useEffect, useState } from 'react';
import { Link, useParams } from 'react-router-dom';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faCaretDown } from '@fortawesome/free-solid-svg-icons';

function ProductPageskincares() {
  const [items, setitems] = useState(null);
  const [information, setInformation] = useState(false);
  const { id } = useParams();

  useEffect(() => {
    fetch(`http://localhost:3000/admin/skincaresshow-product/${id}`)
      .then(res => res.json())
      .then(data => {
        ...
        {items ? (
          <>
            <div className="product-images">
              ...
              <button><Link to={`/admin/skincares/skincaresedit-product/${id}`}>Edit Product</Link></button>
              <button><Link to={`/admin/skincaresdelete-product/${id}`}>Delete Product</Link></button>
            </div>
            ...
            <div className="product-info">
              ...
              {information && (
                <div className="details-grid">
                  ...
                  <div className="row"><span className="labels">Skin-Cares Type:</span><span className="values">{items.productType}</span></div>
                  ...
                  <div className="row"><span className="labels">Concern:</span><span className="values">{items.concern}</span></div>
...
export default ProductPageskincares;
