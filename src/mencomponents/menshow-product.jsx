...
import './menshow-product.css';
import { useEffect, useState } from 'react';
import { Link, useParams } from 'react-router-dom';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faCaretDown } from '@fortawesome/free-solid-svg-icons';

function ProductPagemen() {
  const [items, setitems] = useState(null);
  const [information, setInformation] = useState(false);
  const { id } = useParams();

  useEffect(() => {
    fetch(`http://localhost:3000/admin/menshow-product/${id}`)
      .then(res => res.json())
      .then(data => {
        ...
        {items ? (
          <>
            <div className="product-images">
              ...
              <button><Link to={`/admin/men/menedit-product/${id}`}>Edit Product</Link></button>
              <button><Link to={`/admin/mendelete-product/${id}`}>Delete Product</Link></button>
            </div>
