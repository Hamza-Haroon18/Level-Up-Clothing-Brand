import './menadd-product.css'
import { useState, useRef, useEffect } from 'react'

import { useParams } from 'react-router-dom';
function Editproductmen({ }) {
    const { id } = useParams();
    ...
    useEffect(() => {
        fetch(`http://localhost:3000/admin/men/menedit-product/${id}`)
            .then((res) => res.json())
            .then((data) => {
    ...
    const redirectChange = () => {
        window.location.href = '/admin/men';
    };
    ...
    const redirectsubmit = async (e) => {
        ...
        // formData.append('image', file)
        try {
            const res = await fetch(`http://localhost:3000/admin/men/menedit-product/${id}`, {
                method: 'POST',
                body: formData
            })
            if (res.ok) {
